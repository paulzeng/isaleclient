 ...
 * @功能描述：考勤筛选界面
 */
public class AttenceSearchActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.attence_search);
		getWindow().setFeatureInt(Window.FEATURE_CUSTOM_TITLE,
				R.layout.titlebar_small);
		tv_titlebar_title = (TextView) findViewById(R.id.tv_titlebar_title);
		tv_titlebar_title.setText(R.string.attence_search_title);
		...
		types = getResources().getStringArray(R.array.queryattencetypes);
		tv_attence_type = (TextView) findViewById(R.id.tv_attence_type);
				...
				if (CommonUtils.isNotFastDoubleClick()) {
					if (terminalTypeDialog == null) {
						terminalTypeDialog = new CustomPopupDialog(
								AttenceSearchActivity.this, "考勤类型", types,
			...
			public void onClick(View arg0) {
				if (CommonUtils.isNotFastDoubleClick()) {
					if (!tv_attence_type.getText().equals("")) {
						...
						setResult(Constant.RESULT_ATTANCELIST_SEARCH, intent);
						finish();
					} else {
						alertDialog = new CustomAlertDialog(
								AttenceSearchActivity.this);
						alertDialog.show();
						alertDialog.setTitle("提示");
						alertDialog.setMessage("请选择考勤类型");
