...
import com.zjrc.isale.client.ui.adapter.SaleListAdapter;
import com.zjrc.isale.client.ui.adapter.SaleListAdapter.SaleItem;
...
public class SaleListActivity extends BaseActivity implements
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		tv_titlebar_title.setText(R.string.sale_list);
		...
		adapter = new SaleListAdapter(getLayoutInflater());
		...
		lv_list.setOnRefreshListener(new OnRefreshListener() {
			...
			public void onRefresh() {
				if (actualListView.getFirstVisiblePosition() == 0) {
					SaleItem item = (SaleItem) adapter.getItem(0);
					if (item != null) {
						id="";
						sendQuerySaleList(id, "0",false);
					}
				} else if (actualListView.getLastVisiblePosition() == adapter
						.getCount() + 1) {
					SaleItem item = (SaleItem) adapter.getItem(actualListView
							.getLastVisiblePosition() - 2);
					if (item != null) {
						id=item.getSaleid();
						sendQuerySaleList(id, "1",false);
					}
				}
			}
		});
	...
	public void onClick(View view) {
		if (CommonUtils.isNotFastDoubleClick()) {
			...
			case R.id.btn_titlebar_filter:
				intent = new Intent(SaleListActivity.this, SaleSearchActivity.class);
				bundle = new Bundle();
				...
				startActivityForResult(intent, Constant.SALELIST_REFRESH);
				break;
			case R.id.btn_titlebar_add:
				...
				intent.putExtras(bundle);
				intent.setClass(SaleListActivity.this, SaleSubmitActivity.class);
				startActivityForResult(intent, Constant.SALELIST_REFRESH);
				break;
			}
		}
	...
	public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
		SaleItem item = (SaleItem) adapter.getItem(arg2 - 1);
		if (item != null) {
			Intent intent = new Intent(SaleListActivity.this,
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == Constant.SALELIST_REFRESH) {
	...
	public void sendQuerySaleList(String orderid, String order,boolean needProgress) {
        ...
        if (application != null) {
            ...
            params.put("userid", XmlValueUtil.encodeString(application.getConfig()
                    .getUserid()));
            params.put("saledate", XmlValueUtil.encodeString(saledate));
            ...
            params.put("saleid",   XmlValueUtil.encodeString(orderid));
            params.put("order",XmlValueUtil.encodeString(order));
            request("corpsale!list?code=" + Constant.SALE_LIST, params, FLAG_SHOW_PROGRESS | FLAG_SHOW_ERROR | FLAG_CANCEL);
        }
	}
    ...
    public void onBackPressed() {
        if (!isEmpty(terminalid, terminalcode, terminalname, saledate)) {
            terminalid = "";
            terminalcode = "";
            terminalname = "";
            saledate = "";
    ...
    public void onRecvData(JsonObject response) {
        if (response != null) {
            ...
            if (Constant.SALE_LIST.equals(code)) {
                if (TextUtils.isEmpty(id)) {
                    adapter.clearItem();
                }
                JsonArray sales = response.getAsJsonArray("body");
                if (sales != null) {
                    for (JsonElement brandElem : sales) {
                        JsonObject saleObj = (JsonObject) brandElem;
                        adapter.addItem(saleObj.get("id").getAsString(),
                                saleObj.get("terminalname").getAsString() ,
                                saleObj.get("totalprice").getAsString() ,
                                saleObj.get("saledate").getAsString());
                    }
