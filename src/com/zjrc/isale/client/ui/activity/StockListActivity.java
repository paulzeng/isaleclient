...
import com.zjrc.isale.client.ui.adapter.StockListAdapter;
import com.zjrc.isale.client.ui.adapter.StockListAdapter.StockItem;
...
public class StockListActivity extends BaseActivity implements
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		tv_titlebar_title.setText(R.string.stock_list);
		...
		adapter = new StockListAdapter(getLayoutInflater());
		...
		lv_list.setOnRefreshListener(new OnRefreshListener() {
			...
			public void onRefresh() {
				if (actualListView.getFirstVisiblePosition() == 0) {
					StockItem item = (StockItem) adapter.getItem(0);
					if (item != null) {
						id= "";
						sendQueryStockList(id, "0",false);
					}
				} else if (actualListView.getLastVisiblePosition() == adapter
						.getCount() + 1) {
					StockItem item = (StockItem) adapter.getItem(actualListView
							.getLastVisiblePosition() - 2);
					if (item != null) {
						id=item.getStockid();
						sendQueryStockList(id, "1",false);
					}
				}
			}
		});
	...
	public void onClick(View view) {
		if (CommonUtils.isNotFastDoubleClick()) {
			...
			case R.id.btn_titlebar_filter:
				intent = new Intent(StockListActivity.this,
				...
				bundle.putString("stockdate", stockdate);
				intent.putExtras(bundle);
				startActivityForResult(intent, Constant.STOCKLIST_REFRESH);
				break;
			case R.id.btn_titlebar_add:
				...
				intent.putExtras(bundle);
				intent.setClass(StockListActivity.this, StockSubmitActivity.class);
				startActivityForResult(intent, Constant.STOCKLIST_REFRESH);
				break;
			}
		}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == Constant.STOCKLIST_REFRESH) {
	...
	public void sendQueryStockList(String orderid, String order,boolean needProgress) {
        ...
        if (application != null) {
            ...
            params.put("userid", XmlValueUtil.encodeString(application.getConfig()
                    .getUserid()));
            params.put("stockdate", XmlValueUtil.encodeString(stockdate));
            ...
            params.put("stockid",  XmlValueUtil.encodeString(orderid));
            params.put("order",XmlValueUtil.encodeString(order));
            request("corpstock!list?code=" + Constant.STOCK_LIST, params, FLAG_SHOW_PROGRESS | FLAG_SHOW_ERROR | FLAG_CANCEL);
        }
	...
	public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
		StockItem item = (StockItem) adapter.getItem(arg2 - 1);
		if (item != null) {
			Intent intent = new Intent(StockListActivity.this,
    ...
    public void onBackPressed() {
        if (!isEmpty(terminalid, terminalcode, terminalname, stockdate)) {
            terminalid = "";
            terminalcode = "";
            terminalname = "";
            stockdate = "";
    ...
    public void onRecvData(JsonObject response) {
        if (response != null) {
            ...
            if (Constant.STOCK_LIST.equals(code)) {
                if (TextUtils.isEmpty(id)) {
                    adapter.clearItem();
                }
                JsonArray stocks = response.getAsJsonArray("body");
                if (stocks != null) {
                    for (JsonElement stockElem : stocks) {
                        JsonObject stockObj = (JsonObject) stockElem;
                        adapter.addItem(stockObj.get("id").getAsString(),
                                stockObj.get("terminalname").getAsString(),
                                stockObj.get("totalprice").getAsString(),
                                stockObj.get("stockdate").getAsString());
                    }
